...
Lean 4 formalization of the address-algebra core of the paper
...
machine-checks the combinatorial statements of the paper that rest on that address grammar.
...
| `Hsfnlean/Distance.lean` | Thm. exact metric (i)–(iii) | `dist_eq_dval`: the exact distance formula; `graph_connected`; diameter `2m-1` (`dist_le_diam`, `exists_dist_eq_diam`); greedy one-step descent (`exists_step`, `dval_lipschitz`) |
...
| `Hsfnlean/Clique.lean` | Lemma blocks and maximal cliques | every clique lies in one anchored clique or in the seed cell; clique number `N+1` |
...
* Subtraction-free forms are used wherever the paper divides by `N-1`.
